...
\begin{proof}
  \begin{enumerate}[(1)]
    ...
    \item \textbf{[无 Solution]}
      取顶点集 $V = \set{1,2,3,4,5,6,7,8}$, 边集 $E(G)$ 由以下 $12$ 条边组成:
      \begin{itemize}
        \item 两个 $4$-圈: $12, 23, 34, 41$ 与 $56, 67, 78, 85$
          (此处 $ij$ 表示顶点 $i$ 与 $j$ 之间的边);
        \item 连接两部分的 $4$ 条边: $15, 16, 27, 36$。
      \end{itemize}
      将 $1234$ 画成外圈、$5678$ 画成内圈, 再连 $15, 16, 27, 36$,
      可得 $G$ 的一个平面嵌入, 故 $G$ 是平面图。

      其补图 $\overline{G}$ 含 $28 - 12 = 18 = 3 \cdot 8 - 6$ 条边,
      满足简单平面图的上界。
      直接验证可知, $\overline{G}$ 既不含 $K_5$, 也不含 $K_{3,3}$ 的细分,
      由 Kuratowski 定理, $\overline{G}$ 也是平面图。

      \noindent\emph{注.}
      若取 $G$ 为两个互不相交的 $K_4$, 则 $\overline{G} = K_{4,4}$。
      但 $K_{4,4}$ 含有 $K_{3,3}$ 作为子图 (从每部各取 $3$ 个顶点即可),
      因此 $K_{4,4}$ \emph{不是}平面图;
      ``左右各 $4$ 个顶点'' 的画法仍会出现边交叉。
  \end{enumerate}
\end{proof}
%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%
\begin{problem}[着色多项式; 无 Solution]
  设 $C_{n}$ 是 $n$ 个顶点的圈图。
...
%%%%%%%%%%%%%%%
\begin{problem}[图着色的应用; 无 Solution]
  现在需要为七场讲座制定一份时间表。
