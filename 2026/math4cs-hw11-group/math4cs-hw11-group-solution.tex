...
\date{2026年6月11日 发布作业 \qquad 2026年6月19日 发布答案}
%%%%%%%%%%%%%%%%%%%%
\begin{document}
\maketitle
%%%%%%%%%%%%%%%%%%%%
...
\begin{proof}
  记 $K = gHg^{-1}$。
  由子群判定定理, 只需证明 $K \neq \emptyset$, 且对任意 $x, y \in K$ 有 $xy^{-1} \in K$。

  因为 $e \in H$, 所以 $g e g^{-1} = e \in K$, 故 $K \neq \emptyset$。

  设 $x = gh_1g^{-1}$, $y = gh_2g^{-1}$, 其中 $h_1, h_2 \in H$。
  则
  \[
    xy^{-1}
    = (gh_1g^{-1})(gh_2g^{-1})^{-1}
    = gh_1g^{-1} \cdot g h_2^{-1} g^{-1}
    = g(h_1 h_2^{-1})g^{-1}.
  \]
  因为 $H \le G$, 所以 $h_1 h_2^{-1} \in H$, 从而 $xy^{-1} \in K$。
...
\begin{proof}
  ...
  \[
    \langle d \rangle = \set{0, d, 2d, \dots, (|H|-1)d}
  \]
