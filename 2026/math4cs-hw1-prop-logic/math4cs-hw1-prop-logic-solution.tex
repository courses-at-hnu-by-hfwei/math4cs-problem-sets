...
\begin{proof}
  ...
  \begin{intention}
    不再证明精确公式 $\ell(\alpha) = 4 s(\alpha) - 3$,
    而是只证明推出结论所需的不等式
    $s(\alpha) > \ell(\alpha)/4$。
  \end{intention}

  令 $c(\alpha)$ 表示公式 $\alpha$ 中二元联词的出现次数。
  由于 $\alpha$ 中不含 $\lnot$,
  故 $\alpha$ 的非命题符号恰好由二元联词与配对括号组成。
  因而
  \[
    \ell(\alpha) = s(\alpha) + c(\alpha) + 2 c(\alpha)
    = s(\alpha) + 3 c(\alpha).
  \]
  所以只需证明
  \[
  ...
  \[
    c(\beta) \le s(\beta) - 1,
    \qquad
    c(\gamma) \le s(\gamma) - 1.
  \]
  又
  \[
    c(\alpha) = c(\beta) + c(\gamma) + 1,
    \qquad
    s(\alpha) = s(\beta) + s(\gamma).
  \]
  因而
  ...
  \begin{remark}
    ...
    \textbf{补充视角.} 若只想得到题目结论, 证明 $c(\alpha) < s(\alpha)$ 也足够。
