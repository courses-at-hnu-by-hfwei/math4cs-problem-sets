...
\date{2026年5月15日 发布作业 \qquad 2026年6月19日 发布答案}
%%%%%%%%%%%%%%%%%%%%
\begin{document}
\maketitle
%%%%%%%%%%%%%%%%%%%%
...
\begin{proof}
  \begin{enumerate}[(1)]
    \item \textbf{[无 Solution]}
      闭道路两端相同, 故可设 $u = v$。
      将 $W$ 写为顶点序列
      \[
        W = (v_0, v_1, \ldots, v_k), \qquad v_0 = v_k = u,
      \]
      其中 $k$ 为所含边数 (即 $W$ 的长度)。
      对 $k$ 作强数学归纳, 证明每个这样的 $u\-u$ 闭道路都包含一条 $u\-u$ 路径。

      \begin{description}
        \item[基础步骤:] $k = 0$ 时, $W = (u)$, 本身就是一条 $u\-u$ 路径。
        \item[归纳假设:] 对每个 $k' < k$,
          每个长度为 $k'$ 的 $u\-u$ 闭道路都包含一条 $u\-u$ 路径。
        \item[归纳步骤:] 设 $W = (v_0, v_1, \ldots, v_k)$ 是长度为 $k \ge 1$ 的 $u\-u$ 闭道路。
          若 $v_0, v_1, \ldots, v_{k-1}$ 两两不同, 则 $W$ 中除首尾外无重复顶点,
          故 $W$ 本身就是一条 $u\-u$ 路径。

          否则, 在序列 $v_0, v_1, \ldots, v_{k-1}$ 中
          (不把终点 $v_k = v_0$ 视为一次新的出现),
          取最小的 $j \in \set{1, 2, \ldots, k-1}$,
          使得 $v_j$ 在 $v_0, v_1, \ldots, v_{j-1}$ 中已出现过;
          再取满足 $v_i = v_j$ 的最小指标 $i$, 令 $x = v_i = v_j$。
          于是 $0 \le i < j \le k-1$, 且 $x$ 是 $W$ 中除首尾 $u$ 外首次重复出现的顶点。

          在 $W$ 中, $x$ 的第 $i$ 次与第 $j$ 次出现把 $W$ 拆成两条 $x\-x$ 闭道路:
          \[
            W_1 = (v_i, v_{i+1}, \ldots, v_j),
            \qquad
            W_2 = (v_j, v_{j+1}, \ldots, v_{k-1}, v_k, v_1, \ldots, v_i).
          \]
          二者都是 $W$ 的连续子序列, 因而也是 $G$ 中的 $x\-x$ 闭道路。
          它们的长度分别为
          \[
            |W_1| = j - i \ge 1,
            \qquad
            |W_2| = (k - j) + i \ge 1,
          \]
          且 $|W_1| + |W_2| = k$, 故 $|W_1|, |W_2| < k$。

          由归纳假设, $W_1$ 包含一条 $x\-x$ 路径 $P_1$。
          因 $W_1$ 在 $v_i, \ldots, v_j$ 中已有重复顶点 $x$, 故 $P_1$ 的长度严格小于 $|W_1|$。
          把 $W$ 中子道路 $(v_i, \ldots, v_j)$ 替换为 $P_1$, 得到新的 $u\-u$ 闭道路 $W'$:
          \[
            W' = (v_0, \ldots, v_i) \circ P_1 \circ (v_{j+1}, \ldots, v_k),
          \]
          其中 ``$\circ$'' 表示首尾相同的顶点序列拼接。
          于是
          \[
            |W'| = i + |P_1| + (k - j) < i + (j - i) + (k - j) = k.
          \]
          再由归纳假设, $W'$ 包含一条 $u\-u$ 路径 $P$。
          而 $P$ 也是 $W$ 的子道路 (shortcut 只删去 $W$ 内部的圈, 不删 $P$ 的边),
          故 $W$ 包含一条 $u\-u$ 路径。
      \end{description}
...
\begin{proof}
  ...
  该图可看作广义 Petersen 图 $GP(8, 2)$:
  外层 8 个顶点构成圈 $u_0u_1\cdots u_7u_0$,
  内层 8 个顶点通过 ``隔 2 连边'' 分成两个圈
  $v_0v_2v_4v_6v_0$ 与 $v_1v_3v_5v_7v_1$,
  且每个 $u_i$ 与 $v_i$ 相连。
