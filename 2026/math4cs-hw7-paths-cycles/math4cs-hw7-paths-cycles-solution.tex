...
\begin{proof}
  \begin{enumerate}[(1)]
    \item \textbf{[无 Solution]}
      闭道路两端相同, 故可设 $u = v$。
      设 $W = (v_0, v_1, \ldots, v_k)$ 为长度 $k$ 的 $u\-u$ 闭道路 ($v_0 = v_k = u$)。
      对 $k$ 作强数学归纳, 证明每个这样的闭道路都包含一条 $u\-u$ 路径。

      \begin{description}
        \item[基础步骤:] $k = 0$ 时, $W = (u)$ 即为一条 $u\-u$ 路径。
        \item[归纳假设:] 每个长度 $k' < k$ 的 $u\-u$ 闭道路都包含一条 $u\-u$ 路径。
        \item[归纳步骤:] 设 $W$ 长度为 $k \ge 1$。
          若 $v_0, \ldots, v_{k-1}$ 两两不同, 则 $W$ 本身就是 $u\-u$ 路径。
          否则, 在 $v_0, \ldots, v_{k-1}$ 中 (不计终点 $v_k = v_0$)
          取首次重复出现的顶点 $x$, 设其两次出现位置为 $v_i$ 与 $v_j$ ($0 \le i < j \le k-1$)。
          则
          \[
            W_1 = (v_i, \ldots, v_j), \qquad
            W_2 = (v_j, \ldots, v_k, v_1, \ldots, v_i)
          \]
          是两条更短的 $x\-x$ 闭道路, 且 $|W_1|, |W_2| < k$。
          由归纳假设, $W_1$ 含 $x\-x$ 路径 $P_1$, 其长度严格小于 $|W_1|$。
          将 $W$ 中 $(v_i, \ldots, v_j)$ 替换为 $P_1$, 得长度 $< k$ 的 $u\-u$ 闭道路 $W'$。
          再由归纳假设, $W'$ 含 $u\-u$ 路径, 它也是 $W$ 的子道路, 故 $W$ 含 $u\-u$ 路径。
      \end{description}
