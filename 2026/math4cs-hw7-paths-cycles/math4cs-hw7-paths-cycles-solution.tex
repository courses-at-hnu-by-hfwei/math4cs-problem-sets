...
\begin{proof}
  ...
  \begin{enumerate}[(1)]
    ...
    \item 若 $n$ 为偶数, 则 ``距离为 2'' 的边不再单独构成一条 Hamilton 圈,
      需要把 ``距离为 1'' 与 ``距离为 2'' 的边混合使用。
      此时可采用如下交替构造 (以 $n = 2m$ 为例):
      \[
        H_2 = (v_0, v_2, v_4, \dots, v_{n-2}, v_{n-1}, v_{n-3}, \dots, v_1, v_0).
      \]
      直观地说, 先在偶数顶点间以步长 $2$ 前进, 再用一条 ``距离为 1'' 的边跳到 $v_{n-1}$,
      再在奇数顶点间以步长 $-2$ 返回, 最后回到 $v_0$。
      直接验算可知 $H_2$ 是 Hamilton 圈, 且与 $H_1$ 边不交。
      因此仍有 $E(G_n) = E(H_1) \sqcup E(H_2)$。
  \end{enumerate}
